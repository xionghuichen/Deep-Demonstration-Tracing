# Created by xionghuichen at 2022/12/28
...
class AttenRepresentationBlock(torch.nn.Module):
    def __init__(self, d_model, nhead, dropout, batch_first, dim_feedforward, activation,
                 device=None, dtype=None, layer_norm_eps: float = 1e-5,):
        super().__init__()
    ...
    def forward(self, input: Tensor):
        if len(input.shape) > 2:
            ...
            input = torch.reshape(input, [-1, 1, input.shape[-1]])  # set len to 1
        else:
            input = torch.reshape(input, [-1, 1, input.shape[-1]])  # set len to 1
            original_shape = input.shape[:-1]
        x = self.self_attn(input, input, input, need_weights=False)[0]
        atten_out = self.dropout1(x)
        x = self.norm1(x + atten_out)
        ff_out = self.linear2(self.dropout2(self.activation(self.linear1(x))))
        x = self.norm2(x + ff_out)
        x = torch.reshape(x, list(original_shape) + [x.shape[-1]])
        return x
...
class Transformer(Module):
    ...
    def forward(self, src: Tensor, tgt: Tensor, src_mask: Optional[Tensor] = None, tgt_mask: Optional[Tensor] = None,
        ...
        output, attention_wei_lst = self.decoder(tgt, memory, tgt_mask=tgt_mask, memory_mask=memory_mask,
                              tgt_key_padding_mask=tgt_key_padding_mask,
                              memory_key_padding_mask=memory_key_padding_mask)
        return output, attention_wei_lst
...
class TransformerDecoder(Module):
    ...
    def forward(self, key: Tensor, value: Tensor, query: Tensor, tgt_mask: Optional[Tensor] = None,
        ...
        """
        output = query
        attention_weight_lst=[]
        output, atten_wei = self.layers[0](key, value, output, use_sa=False, tgt_mask=tgt_mask,
                         memory_mask=memory_mask,
                         tgt_key_padding_mask=tgt_key_padding_mask,
                         memory_key_padding_mask=memory_key_padding_mask, )
        attention_weight_lst.append(atten_wei)
        for mod in self.layers[1:]:
            output, atten_wei = mod(key, value, output, tgt_mask=tgt_mask,
                         memory_mask=memory_mask,
                         tgt_key_padding_mask=tgt_key_padding_mask,
                         memory_key_padding_mask=memory_key_padding_mask)
            attention_weight_lst.append(atten_wei)
        ...
        return output, attention_weight_lst
...
class TransformerDecoderLayer(Module):
    ...
    def forward(self, key: Tensor, value: Tensor, query: Tensor, use_sa=True, tgt_mask: Optional[Tensor] = None, memory_mask: Optional[Tensor] = None,
                tgt_key_padding_mask: Optional[Tensor] = None, memory_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ...
        if self.norm_first:
            ...
            mha_output, mha_wei=self._mha_block(self.norm2(x), key, value, memory_mask, memory_key_padding_mask)
            x = x + mha_output
            x = x + self._ff_block(self.norm3(x))
        else:
            if use_sa:
                x = self.norm1(x + self._sa_block(x, tgt_mask, tgt_key_padding_mask))
            mha_output, mha_wei=self._mha_block(x, key, value, memory_mask, memory_key_padding_mask)
    ...
    def _mha_block(self, query: Tensor, key: Tensor, value: Tensor,
                   attn_mask: Optional[Tensor], key_padding_mask: Optional[Tensor]) -> Tensor:
        x, atten_wei = self.multihead_attn(query, key, value,
                                attn_mask=attn_mask,
                                key_padding_mask=key_padding_mask,
                                need_weights=True)
        return self.dropout2(x), atten_wei
