...
def _refresh_maze_local_view(data_pkg, raw_rew_env):
    """
    ...
    new_data_pkg["all_trajs"], new_data_pkg["all_maps"], new_data_pkg["map_id_lst"] = (
        new_all_trajs,
        all_maps,
        map_id_lst,
    )
...
def _get_raw_data_pkg(raw_data_file_path, env_for_refresh):
    """
    Get raw data package from file or refresh it if necessary
    """
    data_file_suffix = (
        f"(ln_{env_for_refresh.local_view_num}_ld_{env_for_refresh.local_view_depth})"
    )
...
def get_dataset(
    ...
    if multi_map:
        ...
        test_map_ids = np.random.choice(
            all_map_id_list, size=unseen_map_num, replace=False
        )
        train_map_ids = list(set(all_map_id_list) - set(test_map_ids))
        iid_tasks = np.concatenate(
            [
                np.random.choice(
                    map_to_task_dict[mid], size=task_num_per_map, replace=False
                )
