...
from eval.metaworld import eval_policy as eval_policy_metaworld, test_unseen as test_unseen_metaworld
...
class BasicTrainer:
    ...
    def update_stats_after_episode(self, task_id, succeed, obstacle_success=None):
...
class ROBOTTrainer(BasicTrainer):
    ...
    def collect_demonstrations(self, configs):
        ...
        if configs['task'] != 'metauniform':
            dataPath = f'./data/robotics_demonstrations/{configs["task"]}-clip-filt'
            ...
            if  fileIdx >= max_data_num:
                ...
                if configs['task'] == 'metauniform':
                    ...
                    if temp_env == "pick-place-v2":
                        dataPath = f'./data/robotics_demonstrations/pick-place-v2-clip-filt'
                    elif temp_env == "pick-place-wall-v2":
                        dataPath = f'./data/robotics_demonstrations/pick-place-wall-v2-clip-filt'
                    else:
                        assert temp_env == "pick-place-hole-v2"
                        dataPath = f'./data/robotics_demonstrations/pick-place-hole-v2-clip-filt'
                    print(dataPath)
    ...
    def evaluation(self, task_id, runned_episodes, task_config=None):
        ...
        if (runned_episodes[task_id]) % self.configs["eval_freq"] == 0: # self.configs["eval_freq"] !!!!!!!!!!!!!!!!!!!!!!!
            ...
            with p:
                _, eval_traj = self.policy.traj_buffer.random_sample(0)
                avg_return = eval_policy_metaworld(
                ...
                if self.configs['obstacle_prob']>0:
                    _, eval_traj = self.policy.traj_buffer.random_sample(0)
                    avg_return_with_obstacle = eval_policy_metaworld(
        ...
        if sum(runned_episodes.values()) % self.configs["eval_unseen_freq"] == 0: # self.configs["eval_unseen_freq"]
            ...
            with p:
                ...
                unseen_result, unseen_ret_dict = test_unseen_metaworld(
                ...
                if self.configs['obstacle_prob']>0:
                    iid_test_ids_choose = random.sample(self.iid_test_ids,
                                                        k=min(unseen_test_num, len(self.iid_test_ids)))
                    unseen_result_with_obstacle, unseen_ret_dict = test_unseen_metaworld(
                        [self.all_trajs, self.all_mujoco_infos, self.all_collect_goals, self.all_task_labels], iid_test_ids_choose, self.policy, self.configs, logger,
